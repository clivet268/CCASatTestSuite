...
for i in range(1, len(perterbedTimes)):
    randomval=random_array[i]
    deltaack = perterbedTimes[i] - perterbedTimes[i - 1]
    
    # To calculate wholly jittered next values
    # this would be equivilent
    deltaack = randomval
    #print(f"{max(perterbedTimes[i - 1], deltaack + 100) - perterbedTimes[i -1]}")
    jitteredRTTs.append(max(perterbedTimes[i - 1], deltaack) - perterbedTimes[i -1])
    perterbedTimes[i] = max(perterbedTimes[i - 1], deltaack)
...
print(max(jitteredRTTs))
plt.title('Old timeline vs Old RTT computed timeline')
print(min(min(oldRTTs), min(jitteredRTTs)))
print(max(max(oldRTTs), max(jitteredRTTs)))
plt.axis((1, len(oldRTTs), min(min(jitteredRTTs), min(oldRTTs)), max(max(jitteredRTTs), max(oldRTTs))))
plt.plot(list(range(1, len(jitteredRTTs) + 1)), jitteredRTTs, color='red', marker=',', label='stuff')
